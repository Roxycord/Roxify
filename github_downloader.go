...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("ROXYCORD_DEV_INSTALL") == "1"
	...
	RoxycordFile := RoxycordDirectory

	stat, err := os.Stat(RoxycordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		RoxycordFile = path.Join(RoxycordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(RoxycordFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Roxycord Install. Checking for hash...")

	re := regexp.MustCompile(`// Roxycord (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(RoxycordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", RoxycordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", RoxycordDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(RoxycordDirectory)
