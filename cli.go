...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Roxycord")
	var updateFlag = flag.Bool("repair", false, "Repair Roxycord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Roxycord")
	...
	if *versionFlag {
		fmt.Println("Roxify Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 Vendicated, thororen1234, Vencord, and Roxycord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Roxify' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Roxycord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Roxycord will likely not work.")
}
