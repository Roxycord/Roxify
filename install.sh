...
if grep -q "CHROMEOS_RELEASE_NAME" /etc/lsb-release 2>/dev/null; then
	echo "ChromeOS is not supported! Use the chrome extension. https://chromewebstore.google.com/detail/roxycord-web/mcambpfmpjnncfoodejdmehedbkjepmi"
	exit 1
fi
...
curl -sS https://github.com/Roxycord/Roxify/releases/latest/download/RoxifyCli-Linux \
  --output "$outfile" \
  --location \
  --fail
