...
var RoxycordDirectory string

func init() {
	if dir := os.Getenv("ROXYCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using ROXYCORD_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../RoxycordData")
		BaseDir = path.Join(dir, "..", "RoxycordData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("Roxycord").UserConfig()
	}
	dir := os.Getenv("ROXYCORD_DIRECTORY")
	if dir == "" {
	...
	if dir != "" {
		Log.Debug("Using ROXYCORD_DIRECTORY")
		RoxycordDirectory = dir
	} else {
		RoxycordDirectory = path.Join(BaseDir, "roxycord.asar")
	}
}
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := WriteAppAsar(appAsar, RoxycordDirectory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", RoxycordDirectory+"...")
		...
		args = append(args, "override", name, "--filesystem="+RoxycordDirectory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + RoxycordDirectory + ": " + err.Error())
		}
	}
	return nil
}
